...
flightsJoined2 %>%
  ...
  summarize(N = sum(flight), avg_arr_delay = mean(arr_delay), avg_dist_km = mean(distance_km)) %>%
...
> ANSWER: Narrow has to do with the format you're seeing the information in. There are wide and narrow formats available to choose.

```{r}
BabyNarrow <- babynames2 %>%
  group_by(name, sex) %>%
  spread(key = sex, value = (sum(n)), fill = 0)
```
