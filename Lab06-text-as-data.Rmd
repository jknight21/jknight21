...
> ANSWER: Stop words are commonly used words and tf-idf decreases the weight of commonly used words.
...
> ANSWER: I would be curious to see how they assign words to scores. While "slick" can have a positive connotation, it does not always. One example is the sentence "The sidewalk was slick today."
...
> ANSWER: Each row represents a sentiment associated with a certain word in the dictionary. Words do not necessarily have just one sentiment attached to them.
...
> ANSWER:  Yes, Emily Dickinson's poems were written in the 1800s. There are slight variations in the English language when she was alive vs now.

- The lexicons are based on unigrams.  Do you see any disadvantages of basing the sentiment on single words?

> ANSWER: Yes, words can change meaning depending on the words that immediately precede or follow it.
...
> ANSWER: A few of the words not scored are "robin", "stood", and "tongue". Around 57% of unigrams within Dickinson's poetry are not scored by this lexicon.
...
```{r}
lexiconbarplot <- ggplot(data = nrcplot, aes(x = word, y = n)) + geom_bar(stat = "identity") + facet_grid(. ~ sentiment)
lexiconbarplot
```

How might one summarize the sentiment of this corpus using the AFINN lexicon?

> ANSWER: There seem to be less words corresponding to anger and more corresponding to anticipation and joy.
