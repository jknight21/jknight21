...
> ANSWER: This code shows a bar graph showing the average charges for medical procedures in New Jersey. The command 'reorder(drg, mean_charge)' sorts each medical procedure by average cost, letting us see each procedure in order of cost. When I use 'x = drg', it is not ordered by average cost, and we don't see the procedures listed in that order.
...
> ANSWER: The theme line alters the text and labels of a graphic by putting it in a more legible format.

```{r}
p <- ggplot(data = ChargesNJ, aes(x = reorder(drg, mean_charge), y = mean_charge)) + geom_bar(fill = "gray", stat = "identity") + ylab("Statewide Average Charges ($)") + xlab("Medical Procedure (DRG)")
...
> ANSWER: What does it mean when you set stat to 'identity'? What does #b2d7e9 mean?
