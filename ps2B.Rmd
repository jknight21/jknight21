...
> ANSWER: 10 pitchers meet the criteria of having at least 300 wins and at least 3,000 strikeouts.
...
> ANSWER: The pitcher with player ID "johnsra05 had the most accumulated strikeouts. He accumulated 4,875 strikeouts. The most strikeouts he had in one season was in 2001, when he had 372.
...
> ANSWER:  The median inspection score does tend to go down when there are an increased number of inspections, however when they are a low number of inspections, there does not seem to be any type of correlation.

```{r}
library(mdsr)
violations1 <- Violations %>%
  filter(boro == "MANHATTAN") %>%
  select(dba, boro, zipcode, score) %>%
  group_by(dba, zipcode) %>%
  summarize(numberofinspections = n(), medianscore = median(score)) %>%
  arrange(medianscore)

visualization <- ggplot(violations1, aes(numberofinspections, medianscore))

visualization + geom_point()
```
...
visualization + geom_point() + geom_segment(aes(x = 75, y = 50, xend = 35, yend = 60), arrow = arrow(length = unit(0.5, "cm"))) + annotate("text", x = 75, y = 50, label = "Kyle Restaurant")
...
FakeDataWide <- spread(FakeDataLong, sex, meanL)
