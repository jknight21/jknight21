...
```{r}
coverage <- read.csv("data/healthcare_coverage.txt")
spending <- read.csv("data/healthcare_spending.txt")
```
...
```{r}
spending <- spending %>%
  select(starts_with(c("X2013", "X2014", "Location")))
coverage <- coverage %>%
  select(starts_with(c("X2013", "X2014", "Location")))
```
...
> RESPONSE: We will need the TotalSpending variable from the spending dataset (from both 2013 and 2014) and the TotalCoverage variable from the coverage dataset (from both 2013 and 2014). 

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: We will need to merge the two datasets together to create a scatterplot to analyze the relationship.
...
```{r}
spending <- spending %>%
  inner_join(coverage, by = c("Location" = "Location"))
spending <- spending %>%
  gather(key = YearMeasure, value = Value, -Location)
```
