...
> ANSWER: This visualization shows the proportion of country-wide hate crimes based on the share of the population that voted for Donald Trump. We can see that generally states in the Midwest and in the South had large proportions of hate crimes compared to states on the West or East coasts. This tracks, as a lot of the states that swung for Trump this election cycle were in these areas with high proportions of Trump voters.

```{r}
hate_crimes <- fivethirtyeight::hate_crimes %>%
  mutate(region = tolower(state))

usa_map_hate_crimes <- hate_crimes %>%
  right_join(usa_states, by = "region")

ggplot(usa_map_hate_crimes, aes(x = long, y = lat, group = group
                      , fill = share_vote_trump)) +
  geom_polygon(color = "white") +
  theme_void() +
  coord_fixed(ratio = 1.3) +
  labs(fill = "Country-wide hate crimes based on the share of the population that voted for Trump") + theme(legend.position = "bottom")
```
...
> ANSWER: This is a map of all counties in Massachusetts, divided by subregion. One challenge I had was getting RStudio to fill in by subregion.

```{r}
ma_counties <- map_data(map = "county", region = "massachusetts")

ggplot(ma_counties, aes(x = long, y = lat, group = group, fill = subregion)) + geom_polygon(color = "white") + theme_void() + coord_fixed(ratio = 1.3) + coord_map(projection = "albers", parameters = c(lat0 = 41, lat1 = 42), wrap = TRUE)
