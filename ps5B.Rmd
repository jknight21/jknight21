...
a. Confirm (using an R command) that the following Wikipedia page allows automated scraping:
...
> ANSWER: The majority of supreme court justices serve somewhere around 8 and 10 years or around 15 years. However, there is a pretty even spread showing the amount of time a justice is likely to serve is extremely variable.
...
```{r}
justices_histogram <- ggplot(justices2, aes(x = tenure_length)) + geom_histogram()
justices_histogram
```
...
```{r, linewidth = 60}
quotes_dat <- read_csv("quotes.csv")
